// app/trip/[id].tsx (TripDetailScreen) — merged: keep your UI, add teammate fixes (tripId parsing + Edit button)
...
export default function TripDetailScreen() {
  ...
  const pendingPaymentsToMe =
    trip.payments?.filter((p: any) => p.status === 'pending' && p.toUser?.id === user?.id) || [];
  const confirmedPayments = trip.payments?.filter((p: any) => p.status === 'confirmed') || [];
  const pendingPaymentsFromMe =
    trip.payments?.filter((p: any) => p.status === 'pending' && p.fromUser?.id === user?.id) || [];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.chipRow}>
          {trip.location && (
            <View style={styles.chip}>
              <Text style={styles.chipText}>📍 {trip.location}</Text>
            </View>
          )}
          <View style={styles.chip}>
            <Text style={styles.chipText}>{trip.people.length} travelers</Text>
          </View>
          <View style={styles.chip}>
            <Text style={styles.chipText}>Total: {formatCurrency(total)}</Text>
          </View>
            ...
            <View style={styles.addMemberSection}>
                ...
                <View style={styles.quickAddSection}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.friendsScroll}>
                    {availableFriends.map((friend) => (
                      <Pressable
                        key={friend.id}
                        style={styles.friendQuickAdd}
                        onPress={() => handleAddMember(friend.username)}
                        disabled={addingMember}
                      >
                        <View style={styles.friendQuickAvatar}>
                          <Text style={styles.friendQuickAvatarText}>{getInitials(friend.username)}</Text>
                        </View>
              ...
              <View style={styles.searchSection}>
                <Text style={styles.quickAddLabel}>
                  {availableFriends.length > 0 ? 'Or search for a user:' : 'Search for a user:'}
                </Text>
                  ...
                  <View style={styles.searchResultsContainer}>
                    {filteredSearchResults.slice(0, 3).map((u) => (
                      <Pressable
                        ...
                        onPress={() => handleAddMember(u.username)}
                        disabled={addingMember}
                      >
                        <View style={styles.searchResultAvatar}>
                          <Text style={styles.searchResultAvatarText}>{getInitials(u.username)}</Text>
                        </View>
          ...
          <View style={styles.membersList}>
            {trip.people.map((person, index) => (
              <View key={index} style={styles.memberChip}>
                <Text style={styles.memberChipText}>{person}</Text>
              </View>
            ))}
          </View>
        </View>
              ...
              <View key={payment.id} style={styles.pendingPaymentCard}>
                ...
                <View style={styles.pendingPaymentActions}>
                  <Pressable
                    ...
                    onPress={() => handleConfirmPayment(payment.id, payment.fromUser?.username)}
                  ...
                  <Pressable
                    ...
                    onPress={() => handleDeclinePayment(payment.id, payment.fromUser?.username, parseFloat(payment.amount))}
                    disabled={respondingTo === payment.id}
                  >
          ...
          <View style={styles.waitingSection}>
            ...
            {pendingPaymentsFromMe.map((payment: any) => (
              <View key={payment.id} style={styles.waitingPaymentCard}>
                ...
                <Text style={styles.waitingPaymentAmount}>
                  {formatCurrency(parseFloat(payment.amount))}
                  {payment.method ? ` via ${payment.method}` : ''}
                </Text>
          ...
          <View style={styles.settlementsSection}>
            ...
            {balancesData.settlements.map((settlement, idx) => (
              <Pressable
                ...
                onPress={() => openPaymentModal(settlement.to.userId, settlement.to.username, settlement.amount)}
              >
                <View style={styles.settlementInfo}>
                  <Text style={styles.settlementText}>
                    <Text style={styles.settlementName}>
                      {settlement.from.userId === user?.id ? 'You' : `@${settlement.from.username}`}
                    </Text>
                    {' → '}
                    <Text style={styles.settlementName}>
                      {settlement.to.userId === user?.id ? 'You' : `@${settlement.to.username}`}
                    </Text>
                  </Text>
                </View>
        ...
        {/* Add Expense */}
        <Pressable
          style={styles.addExpenseBanner}
          onPress={() => router.push(`/new-expense?tripId=${tripId}`)}
        >
            ...
            {paymentTo && (
              <>
                <Text style={styles.payingTo}>
                  Paying <Text style={styles.payingToName}>@{paymentTo.username}</Text>
                </Text>
                ...
                <Pressable
                  style={[styles.recordButton, submittingPayment && styles.recordButtonDisabled]}
